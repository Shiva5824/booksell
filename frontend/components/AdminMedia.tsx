...
import { Upload, Trash2, ChevronUp, ChevronDown, AlertCircle } from "lucide-react";
import { getCarouselMedia, createCarouselMedia, deleteCarouselMedia, updateCarouselMedia, reorderCarouselMedia } from "@/services/site";
...
export default function AdminMediaComponent() {
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const url of urls) {
        ...
        const created = await createCarouselMedia(newMedia);
  ...
  async function moveImage(id: string, direction: "up" | "down") {
    ...
    try {
      await reorderCarouselMedia(updates);
      setMedia(newMedia);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="space-y-8">
        <div className="h-10 w-48 rounded-lg bg-surface-tertiary animate-pulse" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[1, 2].map(i => <div key={i} className="h-64 rounded-lg bg-surface-tertiary animate-pulse" />)}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 font-medium text-ink-secondary text-sm">
          Manage homepage carousel images (notifications, updates, advertisements). Max {maxImages} images.
        </p>
      </div>
      ...
      {!canUpload && (
        <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4 flex items-start gap-3 dark:bg-orange-500/10 dark:border-orange-500/30">
          ...
          <div className="text-sm text-orange-800 dark:text-orange-200">
            <p className="font-bold">Maximum images reached</p>
            <p>Delete an image to upload more</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        {media.length === 0 ? (
          <div className="rounded-2xl bg-white p-12 text-center dark:bg-white/5">
            <p className="text-ink-secondary font-medium">No carousel images yet</p>
          </div>
        ) : (
          media.map((item, idx) => (
            <motion.div
                ...
                <div className="flex-1 min-w-0">
                  {editingId === item._id ? (
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-ink-secondary line-clamp-2">{item.description || "(No description)"}</p>
                      <button
                        onClick={() => {
      ...
      <div className="rounded-2xl bg-blue-50 border border-blue-200 p-4 text-sm text-blue-800 dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-200">
        ...
        <ul className="space-y-1 list-disc list-inside">
          <li>Images should be 610px height or taller for best results</li>
          <li>Auto-slides every 2 seconds (pauses on hover)</li>
          <li>Users can manually navigate with arrow buttons</li>
